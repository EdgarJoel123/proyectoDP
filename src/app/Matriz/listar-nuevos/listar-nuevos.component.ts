...
@Component({
  selector: 'app-listar-nuevos',
  templateUrl: './listar-nuevos.component.html',
  styleUrls: ['./listar-nuevos.component.css']
})
export class ListarNuevosComponent {
  ...
  ngAfterViewInit() {
    this.paginator.page
      .pipe(
        startWith({}),
        switchMap(() => {
          return this.service.getMatrizNuevos();
        }),
        map(data => {
          // Aquí se realiza la paginación de los datos
      ...
      .subscribe(data => {
        ...
        this.paginator.length = this.resultadosBusqueda.length; // Asegúrate de configurar la propiedad "length"
      });
  }
